...
async function main() {
    ...
    try {
        ...
        console.log('Submit a credit transaction.');

        let jsonPayload = {
            id: '2614f7b6-812a-47fd-abae-3e6b04f302fb',
            owner: 'Joao'
        }

        const registerResponse = await contract.submitTransaction('updateCredit', JSON.stringify(jsonPayload));

        let credit = Credit.fromBuffer(registerResponse);

        console.log(`${credit.credit} credits was tranfered to new Owner ${credit.owner} successfully`);
